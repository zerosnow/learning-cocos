...
function BackgroundLayer:createBackgrounds()
	...
	local bg4 = display.newSprite("image/b1.png")
		:align(display.BOTTOM_LEFT, display.left + bg3:getContentSize().width, display.bottom + 10)
		:addTo(self, -2)
...
function BackgroundLayer:scrollBackgrounds(dt)
	...
	local x1 = self.distanceBg[1]:getPositionX() - 100*dt
	local x2 = x1 + self.distanceBg[2]:getContentSize().width
	...
	local x3 = self.nearbyBg[1]:getPositionX() - 200*dt
	local x4 = x3 + self.nearbyBg[2]:getContentSize().width
